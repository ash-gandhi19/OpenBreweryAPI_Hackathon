...
async function Displaydata() {
    ...
    info.forEach((element) => {
        ...
        let address = `${(element.street == null) ? '' : (element.street)} ,${(element.address_2 == null) ? (element.city) : (element.address_2 + ' ' + element.city)}`;
        ...
        <p class='para text-start'><b>Phone:</b> ${(element.phone == null) ? 'Not Available' : element.phone} </p>
        <p class='para text-start'><b>Website Url:</b> ${(element.website_url == null) ? 'Not Available' : element.website_url} </p>
        </div>
        </div>
        `;
    });
